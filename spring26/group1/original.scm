(defprotocol group_one basic 
             (defrole alice 
                      ...
                      (trace 
                        (send (enc (enc time place (privk alice)) (pubk bob)))
                        (recv (enc (enc (cat "Yes" time place) (privk bob)) (pubk alice)))))
             (defrole bob
                      (vars (bob alice name) (time place text))
                      (trace 
                        (recv (enc (enc time place (privk alice)) (pubk bob)))
                        (send (enc (enc (cat "Yes" time place ) (privk bob)) (pubk alice))))))
